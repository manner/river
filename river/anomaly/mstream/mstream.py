...
class MStream(AnomalyDetector):
    ...
    def __init__(self, feature_types, number_buckets=1024, factor=0.8, number_hash_functions=2, timestamp_key='timestamp'):
        # feature_types is just a vector that says True if a feature is categorical, and false if it's numerical
        ...
        self.record_hash = RecordHash(number_buckets, number_hash_functions, feature_types)
        self.feature_hash = FeatureHash(number_buckets, number_hash_functions, feature_types)
        self.factor = factor
        self.is_empty = True
    ...
    def learn_one(self, x):
        ...
        if self.is_empty or timestamp > self.current_time:
            ...
            self.is_empty = False
        self.record_hash.insert(x_numerical, x_categorical)
        self.feature_hash.insert(x_numerical, x_categorical)
        return self
